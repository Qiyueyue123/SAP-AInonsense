...
def search_courses(uid, db):
    # Fetch mentor list from Firestore
    mentor_list = db.collection("courses")
    ...
    for mentor_name, mentor_data in mentor_dict.items():
        mentor_scores = mentor_data['courseScore']
    ...
    user_ref.update({
        "sortedCourses": sorted_mentor_names  # This will create or update the field "sortedMentors"
    })
